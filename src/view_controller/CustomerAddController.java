...
public class CustomerAddController implements Initializable {
    ...
    @FXML
    private Button custAddBtnCancel;
    ...
    void handleAdd() {
        try {
            ...
            if(Customer.isValidCust(currCustomer, currAddress, currCity, currCountry)) {
                CityDB.addCity(currCity);
                currAddress.setCityId(CityDB.getCityId(currCity.getCity()));
                AddressDB.addAddress(currAddress);
                currCustomer.setCustomerAddressId(AddressDB.getAddressId(currAddress.getAddress()));
                ...
                stage.show();
                Stage currStage = (Stage) custAddBtnSave.getScene().getWindow();
                currStage.close();
            }
        }
    ...
    void handleCancel() {
        try {
            ...
            stage.setScene(scene);
            stage.show();
            Stage currStage = (Stage) custAddBtnCancel.getScene().getWindow();
            currStage.close();
        }
    ...
    public void getCustData() {
        try {
            ...
            currCountry.setCountry(country.getSelectionModel().getSelectedItem().getCountry());
        }
